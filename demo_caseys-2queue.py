...
def main(yolo):
    ...
    while True:
        ...
        pts3 = np.array([[133, 120], [303, 120], [297,175], [127, 175]], np.int32)
        ...
        for track in tracker.tracks:
            ...
            if queue_point_test_2 == 'inside':
                ...
                latest_frame[track.track_id] = frame_count
                cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])),(248, 197, 39), 2)
                cv2.rectangle(frame, (int(bbox[0]) - 1, int(bbox[1]) - 15), (int(bbox[2]) + 1, int(bbox[1])), (248, 197, 39), -1)
                wait_time = round((queue_track_dict_2[track.track_id] / Input_FPS), 2)
                cv2.putText(frame, str(wait_time) + "s", (int(bbox[0]), int(bbox[1])), 5, 0.9, (0, 0, 0), 1)
    ...
    for k, v in store_track_dict.items():
         csv_row = {}
         csv_row = {csv_columns[0]: k, csv_columns[1]: round((queue_track_dict_2[k] / Input_FPS), 2), csv_columns[2]: round((v / Input_FPS), 2)}
